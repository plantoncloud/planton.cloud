...
const MenuLabel: FC<TypographyProps> = (props) => {
  return <Typography {...props} className={`${props.className}  text-white text-sm font-medium cursor-pointer`}>
          {props.children}
        </Typography>
}
  ...
  return (
    <>
      <Stack className="flex-row p-2.5 bg-black bg-opacity-10 border-b border-neutral-800 backdrop-blur-md justify-between items-center">
        ...
        <Stack onClick={()=> setOpenMenu(!openMenu)} className="w-8 h-8 items-center justify-center bg-black bg-opacity-10 rounded-[28px] border border-neutral-800 backdrop-blur-md cursor-pointer">
          { openMenu ? <Close fontSize='small'  /> : <DensityMedium fontSize='small'  />}
        </Stack>
      </Stack>
      {openMenu && <Stack className="px-1.5 py-6 bg-black bg-opacity-20 backdrop-blur-xl gap-3">
        ...
        <Box className="px-4 py-2.5">
          <Stack className="items-center justify-between flex-row py-2.5">
            <MenuLabel>
              Products
            </MenuLabel>
            <ArrowDropUp className={`${openProductMenu ? '' : 'rotate-180'} cursor-pointer`} onClick={()=> setOpenProductMenu(!openProductMenu)} />
          </Stack>
          {openProductMenu && <Stack className="justify-start items-start gap-1">
